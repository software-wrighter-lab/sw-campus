...
<!-- sw-campus repo-specific policy below. Keep it OUTSIDE the agentrail
     markers above so `agentrail instructions apply` cannot overwrite it. -->

## What this repo is

The Software Wrighter Research Campus: a Rust/Yew/WASM single-page app
that presents public projects as a painted campus you can walk into
(campus -> building -> wing -> exhibit), every place with its own URL.
Read `docs/plan.md` first; it is the specification. `docs/research.txt`
is the raw design conversation it was distilled from -- input, not spec.
Where they disagree, the plan wins.

`README.md` states the public-facing status. Update it in the same commit
as the work it describes.

## Hard constraints

- **Rust 2024 edition** in every `Cargo.toml`. `sw-checklist` fails otherwise.
- **No hand-written JavaScript, no Python.** Trunk's generated glue is the
  only JS. Asset conversion uses `sips`/`cwebp`/Rust tools, never a Python
  script.
- **SVG in the DOM, not WebGL/canvas.** Hotspots are real `<a>` elements
  with `role`, `tabindex`, `aria-label`.
- **Semantic model never knows geometry.** `campus-model` (places) and
  `campus-scene` (hotspots) stay separate crates with no web dependency;
  both are validated by native `cargo test`.
- **Content is RON in `content/`**, embedded with `include_str!`. Source
  art is `images/*.png` (never served); the web build serves optimised
  copies from `crates/campus-web/assets/`.
- **Clean URLs.** `BrowserRouter` with `basename` read from `<base href>`;
  never hash routing, never a hard-coded `/sw-campus/` in Rust.

## sw-checklist

Run `sw-checklist` before every commit. This repo starts at zero
failures and zero warnings. **Keep it there.** Design to the gate, not
the fail line: <= 25 LOC per function, <= 4 functions per module,
<= 4 modules per crate. When a crate is at 4 modules and the next
feature needs a fifth concern, create the sibling crate FIRST
(`docs/plan.md` names the split: `campus-ui-scene` / `campus-ui-chrome`).

The WASM crate must also pass the Web UI checks: `index.html` next to
`Cargo.toml` referencing `favicon.ico`, and a `<footer>` carrying
...
on its own line with a justification. Use sparingly.
...
which runs, in order: `cargo fmt --all -- --check`, `cargo clippy
--all-targets -- -D warnings`, clippy again for `wasm32-unknown-unknown`,
`cargo test`, `trunk build --release --public-url /sw-campus/`, and
`sw-checklist`. Every command passes before you commit; a red gate means
the step is not done. Then commit, then `agentrail complete`.

## Agentrail in this repo

- `.agentrail/sessions/` is gitignored (raw transcripts, tens of MB).
  Everything else under `.agentrail/` is tracked and committed with the
  step's source changes.
- Step prompts are self-contained but every one begins with "read
  docs/plan.md". Do that; the plan carries the hotspot coordinates,
  content tree and URL rules the prompts refer to.
- Unplanned work goes in via `agentrail insert`, not silently into the
  current step. Future sagas are queued at the end of `docs/plan.md`.

## Commit messages

Imperative subject, <= 72 chars, body says why. End with the attribution
trailers the session provides.
